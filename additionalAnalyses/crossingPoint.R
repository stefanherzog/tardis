...
similarityWeightedBetter <- sapply(1:14000, function(x) ifelse(similarityWeightedCrowd[x,15]>wholeCrowd[x,15],TRUE,FALSE))

#2. clique vs wholeCrowd
cliqueBetter <- sapply(1:14000, function(x) ifelse( clique[x,15]>wholeCrowd[x,15],TRUE,FALSE))

#3. clique vs similarCrowd
cliqueBetter2 <- sapply(1:14000, function(x) ifelse( clique[x,15]>similarCrowd[x,15],TRUE,FALSE))

#4. similarityWeightedCrowd vs similarCrowd
similarityWeightedBetter2 <- sapply(1:14000, function(x) ifelse(similarityWeightedCrowd[x,15]>similarCrowd[x,15],TRUE,FALSE))
...
save(crossingPoint, file="crossingPoint.Rdata")
